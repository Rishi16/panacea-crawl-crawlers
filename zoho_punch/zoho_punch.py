...
class Crawler(Spider):
    def __init__(self, current_path, object=None):
        ...
        menu = (
            ("Check In", self.green_icon, self.punch_in_context),
    ...
    def initiate(self, input_row, region, proxies_from_tool, thread_name):
        while True:
            if (
                ...
                and date.today().strftime("%Y-%m-%d") not in self.holidays
            ):
                            ...
                            # manual check out cope
                                ...
                                if response["response"][-1]["tdate"] != "-":
                                    ...
                                    punch_time = datetime.strptime(
                                        punch_time.strip(), "%d-%b-%Y - %I:%M %p"
                                    )
                                    pickle.dump(
                                        punch_time, open("zoho_punched_out", "wb")
                                    )
                                    if os.path.exists("zoho_punched_in"):
                                        os.remove("zoho_punched_in")
                        else:
                ...
                if os.path.exists("zoho_punched_in"):
                    in_time = pickle.load(open("zoho_punched_in", "rb"))
                    self.logger.info(
                        f"In Time: {in_time.strftime('%m/%d/%Y, %H:%M:%S')}"
                    )
                    self.time_passed = datetime.now() - in_time
                    if (
                        self.time_passed.seconds > 9 * 60 * 60
                        and self.time_passed.days == 0
                    ):
                        ...
                        message = (
                            f'Checked In: {in_time.strftime(" %H:%M")} | Hours: '
                    ...
                    self.logger.info(
                        f"Out Time: {out_time.strftime('%m/%d/%Y, %H:%M:%S')}"
                    )
            ...
            try:
                ...
                if response:
                    ...
                    if punch_type == "punchOut":
                        ...
                        message = (
                            f'Checked Out: {punch_time.strftime(" %H:%M")} | Hours: '
    ...
    def automated_login(self, punch_type=None):
        ...
        data, driver = general.get_url2(
            url="https://people.zoho.com/hrmsbct/zp#home/dashboard",
        ...
        if general.wait_driver(driver, '//a[@class="zgh-login"]', 10):
            ...
            general.wait_driver(
                driver, '//button[@id="nextbtn" and @class="btn blue"]', 60
            )
        ...
        if general.wait_driver(driver, '//div[@class="ZPPimg dropdown"]', 10):
            ...
            general.send_text(
                driver, secrets["email"], '//input[@type="email"]', 0, click=True
            )
            general.wait_driver(driver, '//input[@name="passwd"]', 30)
            time.sleep(5)
            self.logger.info(f"Microsoft Login Password")
            general.send_text(
                driver, secrets["password"], '//input[@name="passwd"]', 0, click=True
            )
            approved = general.wait_driver(
                driver,
        ...
        if general.wait_driver(driver, '//div[@class="ZPPimg dropdown"]', 120):
            ...
            if general.find_elements_driver(
                driver, '//div[@id="ZPD_Top_Att_Stat" and @class="in CP"]'
            ):
                ...
                if punch_type == "punnchIn":
                    ...
                    general.wait_driver(
                        driver, '//div[@id="ZPD_Top_Att_Stat" and @class="out CP"]', 60
                    )
                    message = f'Checked In: {punch_time.strftime(" %H:%M")}'
                    self.logger.info(
                        f"Punch In Successful: {punch_time.strftime('%m/%d/%Y, %H:%M:%S')}"
            ...
            elif general.find_elements_driver(
                driver, '//div[@id="ZPD_Top_Att_Stat" and @class="out ' 'CP"]'
            ):
                ...
                if punch_type == "punchOut":
                    ...
                    general.wait_driver(
                        driver, '//div[@id="ZPD_Top_Att_Stat" and @class="in CP"]', 60
                    )
                    self.logger.info(
                    ...
                    message = (
                        f'Checked Out: {punch_time.strftime(" %H:%M")}'
            ...
            for request in driver.requests:
                if "https://people.zoho.com/hrmsbct/viewPhoto" in request.url:
                    ...
                    csrf = re.findall(r"(?<=CSRF_TOKEN=)[^;]*(?=;)", headers["Cookie"])[
                        0
                    ]
                    pickle.dump(
                        {"CSRF": csrf, "Cookie": headers["Cookie"]},
    ...
    def request_zoho(self, url):
        if "mode" in url:
            ...
            payload = {"mode": "getStatus", "conreqcsr": str(self.zoho_session["CSRF"])}
        headers = {
            "Accept": "*/*",
